...
All proof directives can be included using the `proof:kind` pattern, where the proof directives are shown in [](#proof-list). A plain proof can also be written without a kind, as `{proof}`. The directive is enumerated by default — except the bare proof directive (`proof` / `prf:proof` / `proof:proof`), which is unnumbered by default to match the LaTeX `amsthm` convention (opt in with `:enumerated: true`) — and can take in an optional title argument which is shown in brackets after the proof.
