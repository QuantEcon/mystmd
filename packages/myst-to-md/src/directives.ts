...
function containerValidator(node: any, file: VFile) {
  ...
  if (kind !== 'figure' && kind !== 'table' && kind !== 'code' && kind !== 'quote') {
...
function container(node: any, _: Parent, state: NestedState, info: Info): string {
  ...
  if (node.kind === 'quote') {
    ...
    // Append attribution (caption) as a blockquote line with em-dash prefix
    if (captionNode) {
      const attribution = state.containerPhrasing(captionNode as any, info);
      if (attribution) {
        result += '\n>\n> \u2014 ' + attribution;
      }
    }
